 ...
 * Security: Uses PBKDF2 (via WebCrypto) for key derivation, AES-GCM for encryption, HKDF for key derivation
 * 
 * TEMPORARY: Using PBKDF2 via WebCrypto instead of Argon2 to avoid WASM issues in Tauri.
 * All crypto operations remain local-only and secure.
 */

export interface CryptoParams {
  salt: Uint8Array;
  iterations?: number; // PBKDF2 iteration count (recommended: 200000-400000)
  parallelism?: number; // Unused for PBKDF2, kept for compatibility
}
...
export async function deriveKey(
  ...
  const defaultParams: CryptoParams = params || {
    salt: defaultSalt,
    iterations: 300000, // PBKDF2 iteration count (recommended: 200000-400000)
    parallelism: 1, // Unused for PBKDF2, kept for compatibility
  };

  try {
    // 1. Encode password with TextEncoder
    const passwordBytes = new TextEncoder().encode(password);

    // 2. Import password as a key material for PBKDF2
    ...
    // 3. Ensure salt is a proper ArrayBuffer-backed Uint8Array for TypeScript
    // Create a new Uint8Array to ensure it's backed by ArrayBuffer, not SharedArrayBuffer
    const saltBuffer = new Uint8Array(defaultSalt);
    ...
    const derivedKey = await crypto.subtle.deriveKey(
      {
        ...
        iterations: defaultParams.iterations || 300000,
      },
      passwordKey,
    ...
    console.error('[Crypto] PBKDF2 derivation failed:', error);
