...
import { validateStep, validatePassword, type WizardStepId } from '../lib/validation';
...
const STEPS: { id: WizardStepId; label: string }[] = [
  ...
  { id: 'intent', label: 'Intent & Interests' },
  { id: 'look', label: 'Look & Language' },
  { id: 'privacy', label: 'Privacy & Save' },
  { id: 'review', label: 'Review' },
];

const TOTAL_STEPS = STEPS.length;
const getStepIndex = (id: WizardStepId) => STEPS.findIndex((s) => s.id === id);
...
export default function SetupWizard({ onComplete }: SetupWizardProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        if (!['password', 'roles', 'review'].includes(currentStep.id)) {
  ...
  }, [stepIndex, wizardData, currentStep.id]);
  ...
  const addInterest = (value: string) => {
    ...
    if (current.length >= 12) {
      setErrors(['You can add up to 12 interests']);
      return;
    }
    ...
    setField('interests', [...current, trimmed]);
    setInterestInput('');
  };

  const removeInterest = (value: string) => {
    setField('interests', (interests || []).filter((item) => item !== value));
  };
  ...
  const handleNext = async () => {
    ...
    const validation = validateStep(currentStep.id, { ...wizardData, useType: useType || undefined });
    if (!validation.valid) {
    ...
    if (currentStep.id === 'useType' && useType === 'Personal') {
      // Skip sector, sub-sectors, roles for Personal
      setStep(getStepIndex('prefs')); // Go to prefs
      playSound('click.wav');
      return;
    }
  ...
  const handleBack = () => {
    if (stepIndex > 0) {
      ...
      if (currentStep.id === 'prefs' && useType === 'Personal') {
        setStep(getStepIndex('useType')); // Go back to useType
        return;
      }
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const resolvedSavePreference = savePreference || 'ask';
      const resolvedSecurity = securityLevel || (passwordSet ? 'secure' : 'standard');
      const resolvedLanguage = language || (prefs.spelling === 'UK' ? 'English (UK)' : 'English (US)');
      const resolvedTheme = theme || 'Midnight';
      const resolvedGoal = goal || 'Assist the user intelligently and privately.';
      const idleLockMinutes = resolvedSecurity === 'secure' ? 15 : 60;
      ...
      saveConfig({
        ...
        savePreference: resolvedSavePreference,
        userName: name,
  ...
  const renderStep = () => {
    switch (currentStep.id) {
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  value={prefs.length}
                  onChange={(e) =>
                    setField('prefs', { ...prefs, length: parseInt(e.target.value) as 1 | 2 | 3 | 4 | 5 })
                  }
      ...
      case 'intent':
        return (
          <>
            <h2 className="text-3xl font-bold mb-4">What is your primary intent?</h2>
            <p className="text-gray-400 text-sm mb-6">
              Tell MONAD what outcome you want so responses stay focused.
            </p>
            <textarea
              value={goal}
              onChange={(e) => setField('goal', e.target.value)}
              placeholder="e.g., Ship a product launch plan, write daily reflections, or draft research notes."
              className="w-full p-4 rounded-xl bg-slate-700/50 backdrop-blur-sm text-white text-base focus:outline-none focus:ring-2 focus:ring-blue-400 border border-slate-600 min-h-[120px]"
            />
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {(interests || []).map((chip) => (
                  <span
                    key={chip}
                    className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-700/60 border border-slate-500 text-sm"
                  >
                    {chip}
                    <button
                      type="button"
                      onClick={() => removeInterest(chip)}
              ...
              <input
                ...
                placeholder="Add an interest (e.g., privacy law, robotics)"
                className="w-full p-4 rounded-xl bg-slate-700/50 backdrop-blur-sm text-white text-base focus:outline-none focus:ring-2 focus:ring-blue-400 border border-slate-600"
              />
              <div className="text-xs text-gray-400 mt-2">You can add up to 12 interests.</div>
            </div>
          </>
        );
      case 'look':
        return (
          <>
            <h2 className="text-3xl font-bold mb-4">Choose your look & language</h2>
            <p className="text-gray-400 text-sm mb-6">
              Themes stay local; no external assets are loaded.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              {['Midnight', 'Dark', 'Dim', 'Light'].map((t) => (
                <motion.button
                  key={t}
                  onClick={() => setField('theme', t as typeof theme)}
                  className={`px-4 py-3 rounded-xl border-2 transition-all duration-300 ${
                    theme === t
            ...
            <div>
              <label className="text-lg font-medium mb-3 block">Language preference</label>
              <div className="grid grid-cols-2 gap-3">
                {['English (UK)', 'English (US)'].map((lang) => (
                  <motion.button
                    key={lang}
                    onClick={() => setField('language', lang as typeof language)}
                    className={`px-4 py-3 rounded-xl border-2 transition-all duration-300 ${
                      language === lang
                        ? 'border-blue-400 bg-blue-600'
                        : 'border-slate-600 bg-slate-800/50 hover:border-blue-400/50'
      ...
      case 'privacy':
        return (
          <>
            <h2 className="text-3xl font-bold mb-4">Privacy & saving</h2>
            <p className="text-gray-400 text-sm mb-6">
              Choose how MONAD stores data locally. Nothing ever leaves this device.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
              {[
                { key: 'always', label: 'Always save', desc: 'Persist chats locally' },
                { key: 'ask', label: 'Ask each time', desc: 'Session-only unless you confirm' },
                { key: 'never', label: 'Never save', desc: 'Keep everything in memory' },
              ].map((opt) => (
                <motion.button
                  key={opt.key}
                  onClick={() => setField('savePreference', opt.key as typeof savePreference)}
                  className={`p-4 rounded-xl border-2 text-left transition-all duration-300 ${
                    savePreference === opt.key
                      ? 'border-blue-400 bg-blue-600/30'
                      : 'border-slate-600 bg-slate-800/50 hover:border-blue-400/50'
                  }`}
                  whileHover={{ scale: 1.02 }}
            ...
            <div>
              <label className="text-lg font-medium mb-3 block">Lock behaviour</label>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { key: 'secure', label: 'Secure', detail: 'Password + 15 min idle lock' },
                  { key: 'standard', label: 'Standard', detail: 'Longer idle lock (60 min)' },
                ].map((opt) => (
                  <motion.button
                    key={opt.key}
                    onClick={() => setField('securityLevel', opt.key as typeof securityLevel)}
                    className={`p-4 rounded-xl border-2 text-left transition-all duration-300 ${
                      securityLevel === opt.key
                        ? 'border-blue-400 bg-blue-600'
                        : 'border-slate-600 bg-slate-800/50 hover:border-blue-400/50'
                    }`}
                    whileHover={{ scale: 1.02 }}
                  ...
                  >
                    <div className="font-semibold">{opt.label}</div>
                    <div className="text-sm text-gray-400">{opt.detail}</div>
                  </motion.button>
                ))}
              </div>
            </div>
      ...
      case 'review':
        return (
          <>
            ...
            <p className="text-gray-400 mb-6 text-xl">Ready to start MONAD?</p>
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 mb-6 border border-slate-600 max-h-96 overflow-y-auto text-left">
              <div className="space-y-3 text-sm">
                ...
                <div><span className="text-gray-400">Goal:</span> <span className="text-white">{goal || 'Not set'}</span></div>
                <div><span className="text-gray-400">Interests:</span> <span className="text-white">{(interests || []).length > 0 ? (interests || []).join(', ') : 'None set'}</span></div>
                ...
                <div><span className="text-gray-400">Theme:</span> <span className="text-white">{theme}</span></div>
                <div><span className="text-gray-400">Language:</span> <span className="text-white">{language}</span></div>
                <div><span className="text-gray-400">Save Preference:</span> <span className="text-white">{savePreference}</span></div>
                <div><span className="text-gray-400">Security:</span> <span className="text-white">{securityLevel}</span></div>
              </div>
            </div>
            <button
    ...
    <div className="h-screen w-full flex flex-col justify-center items-center bg-gradient-to-br from-slate-900 via-slate-800 to-black text-white px-8 relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Navigation Buttons */}
          {currentStep.id !== 'password' && currentStep.id !== 'review' && (
